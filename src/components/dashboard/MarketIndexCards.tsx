...
const indexConfig: Record<string, { accent: string; accentBg: string; label: string; exchange: 'DSE' | 'CSE' }> = {
  DSEX: { accent: 'border-l-blue-500', accentBg: 'bg-blue-500/10 text-blue-500', label: 'DSEX', exchange: 'DSE' },
  DSES: { accent: 'border-l-emerald-500', accentBg: 'bg-emerald-500/10 text-emerald-500', label: 'DSES', exchange: 'DSE' },
  DS30: { accent: 'border-l-purple-500', accentBg: 'bg-purple-500/10 text-purple-500', label: 'DS30', exchange: 'DSE' },
  CASPI: { accent: 'border-l-orange-500', accentBg: 'bg-orange-500/10 text-orange-500', label: 'CASPI', exchange: 'CSE' },
  CSE30: { accent: 'border-l-cyan-500', accentBg: 'bg-cyan-500/10 text-cyan-500', label: 'CSE30', exchange: 'CSE' },
  CSES: { accent: 'border-l-rose-500', accentBg: 'bg-rose-500/10 text-rose-500', label: 'CSES', exchange: 'CSE' },
};
...
function IndexCard({ index }: { index: MarketIndex | undefined; name: string }) {
  ...
  return (
    <Card
      className={cn('border-l-4 !rounded-xl !p-0 overflow-hidden', config.accent)}
      padding={false}
    >
      <div className="px-4 py-3.5 sm:px-5 sm:py-4">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[9px] font-semibold text-muted/60 uppercase">{config.exchange}</span>
          </div>
          <div className={cn(
        ...
        <div className="flex items-center justify-between mt-2">
          <span className={cn('text-xs font-medium font-num', isPositive ? 'text-success' : 'text-danger')}>
          ...
          <div className="flex items-end gap-px h-4 opacity-40">
            {[40, 55, 45, 60, 50, 70, 65, 75, 60, 80, 70, 85].map((h, i) => (
              <div
                key={i}
                className={cn('w-[3px] rounded-sm', isPositive ? 'bg-success' : 'bg-danger')}
                style={{ height: `${h}%` }}
              />
            ))}
          </div>
...
export function MarketIndexCards({ indices, stats, exchange = 'ALL' }: Props) {
  ...
  const caspi = indices.find(i => i.index_name === 'CASPI');
  const cse30 = indices.find(i => i.index_name === 'CSE30');
  const cses = indices.find(i => i.index_name === 'CSES');

  const showDSE = exchange === 'ALL' || exchange === 'DSE';
  const showCSE = exchange === 'ALL' || exchange === 'CSE';

  // CSE placeholder indices when not yet in database
  const cseIndices = caspi || cse30 || cses;
        ...
        cseIndices ? (
          ...
          <div className="rounded-xl border border-border bg-card-solid p-4 shadow-[var(--shadow-card)]">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-muted">CASPI &middot; CSE30 &middot; CSES</span>
            </div>
            <p className="text-xs text-muted mt-2">CSE index data will be available once the CSE scraper is integrated. Infrastructure ready.</p>
          </div>
        )
      )}
